...
- **Hotfixes** — **#6** bumped Anthropic model IDs to Claude 4.X family (`claude-sonnet-4-6`, `claude-opus-4-7`, `claude-haiku-4-5-20251001`) after 404 on retired `claude-sonnet-4-20250514`. **#7** promoted Sprint 12 onto main (stacked-PR base mishandling). **#8** Scan ZIPs UX: clamp Top-N 1-15 on blur, auto-expand + scroll to Batch panel on submit, show chosen mode (sync / async) in scan summary.
